...
class DC_and_CL_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, cl_kwargs, aggregate="sum", square_dice=False, weight_cl=0.4, weight_dice=0.6,
                 log_dice=False, ignore_label=None):
        """
        CAREFUL. Weights for CE and Dice do not need to sum to one. You can set whatever you want.
        :param soft_dice_kwargs:
        ...
        :param weight_dice:
        """
        super(DC_and_CL_loss, self).__init__()
    ...
    def forward(self, net_output, target):
        ...
        cl_loss = self.cl(net_output, target) if self.weight_cl != 0 else 0

        if self.aggregate == "sum":
            result = self.weight_cl * (1 + cl_loss) + self.weight_dice * (1 + dc_loss)
        else:
            raise NotImplementedError("nah son")  # reserved for other stuff (later)
        return result
